...
export const getAllServices = async (req, res) => {
  try {
    ...
    const services = rows.map(service => ({
      ...service,
      imagem: service.imagem ? service.imagem.toString() : null
    }));
...
// GET - Buscar serviço por ID (com características e inclusos)
...
export const updateService = async (req, res) => {
  try {
    ...
    await db.query(
      `UPDATE servico SET titulo=?, descricao=?, detalhes=?, preco=?, localidade=?, contato=? WHERE id=?`,
